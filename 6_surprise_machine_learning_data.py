...
frame_eeg = loadtxt('D:\\recording\\frame_eeg_ghost_car.csv', delimiter=',', skiprows=1)
...
total_epochs = numpy.empty([0, 13])	
frame_iterations = len(frame_eeg) - 3
...
for frame in frame_eeg[0:frame_iterations, 0]:
	...
	for i in range (0, 15):
		if (surprise_onset[i] == int(frame)+0 or surprise_onset[i] == int(frame)+1 or surprise_onset[i] == int(frame)+2):
			label = numpy.array([1])
			break
		else:
			label = numpy.array([0])	
	...
	total_epochs = numpy.append(total_epochs, flattened_array.reshape(1, 13), axis=0)

savetxt("total_epochs.csv", total_epochs, delimiter=",")
