...
my_epochs = loadtxt('D:\\recording\\total_epochs.csv', delimiter=',', skiprows=1)
...
input = rScaler.fit_transform(train_data[:, 0:12])
testinput = rScaler.fit_transform(X_test_tmp[:,0:12])
...
input = input.reshape(len(input), 1, 12)
input = input.transpose(0, 2, 1)
print (input.shape)

testinput = testinput.reshape(len(testinput), 1, 12)
testinput = testinput.transpose(0, 2, 1)
print (testinput.shape)

# Create the model
model=Sequential()
model.add(Conv1D(filters=10, kernel_size=4, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2), padding='valid', activation='relu', strides=1, input_shape=(12, 1)))
model.add(Conv1D(filters=10, kernel_size=4, kernel_regularizer=L2(0.001), bias_regularizer=L2(0.001), activity_regularizer = L2(0.001), kernel_constraint=min_max_norm(min_value=-0.2, max_value=0.2), padding='valid', activation='relu', strides=1))
model.add(Flatten())
model.add(Dense(15))
...
mc = ModelCheckpoint('d:\\recording\\best_model.h5', monitor='val_accuracy', mode='max', verbose=1, save_best_only=True)
...
model.save("d:\\recording\\model_conv1d.h5")

# load the saved model
saved_model = load_model('d:\\recording\\best_model.h5')
# evaluate the model
